...
class ImdbSpider(scrapy.Spider):
    ...
    def parse_movie(self, response, movie):
        ...
        movie['runtime_mins'] = (int(
            response.xpath("//li[@data-testid='title-techspec_runtime']/div/text()").getall()[0]) * 60
                                 if 'hours' in response.xpath(
            "//li[@data-testid='title-techspec_runtime']/div/text()").getall()
                                 else 0) + (int(re.findall('(\d+) minutes', ''.join(
            map(str, response.xpath("//li[@data-testid='title-techspec_runtime']/div/text()").getall())))[0])
                                            if 'minutes' in response.xpath(
        ...
        movie['gross_worldwide_usd'] = response.xpath("//li[@data-testid='title-boxoffice-cumulativeworldwidegross']"
                                                      "/div/ul/li/span/text()").get().replace('$',
                                                                                              '') if response.xpath(
            "//li[@data-testid='title-boxoffice-cumulativeworldwidegross']/div/ul/li/span/text()").get() is not None else 0
        return movie
